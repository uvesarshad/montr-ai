...
export async function POST(request: NextRequest) {
    try {
        ...
        const limited = await applyAiRateLimit(request, 'ai:social-ideas', session.user.id!);
        if (limited) return limited;

        const body = await request.json();
        const { topic, industry, platform, count, contentTypes, brandId } = body;
        ...
        return NextResponse.json(
            { error: (error instanceof Error ? error.message : String(error)) || 'Failed to generate ideas' },
            { status: 500 }
        );
    }
}
