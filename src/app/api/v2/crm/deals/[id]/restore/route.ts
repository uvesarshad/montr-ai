...
/**
 * POST /api/v2/crm/deals/[id]/restore
...
export async function POST(_request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    assertCrmPermission(ctx, 'deal', 'delete');
    const user = await userRepository.findById(userId);

    if (!user) {
      return NextResponse.json({ error: 'No organization found' }, { status: 403 });
    }
    const restored = await dealRepository.restore(params.id);
    if (!restored) {
      return NextResponse.json({ error: 'Deal not found in trash' }, { status: 404 });
    }
    ...
    return NextResponse.json(
      { error: 'Failed to restore deal', details: (error instanceof Error ? error.message : String(error)) },
      { status: 500 }
    );
  }
}
